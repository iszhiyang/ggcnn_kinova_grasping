...
grasp_crop = rospy.Publisher('ggcnn/img/grasp_crop', Image, queue_size=1)
...
def depth_callback(depth_message):
    ...
    with TimeIt('Crop'):
        img = np.frombuffer(depth_message.data, dtype=np.uint8)
        print(np.shape(img))
        print(depth_message.height)
        print(depth_message.width)
        # depth = bridge.imgmsg_to_cv2(depth_message)
        depth = np.frombuffer(depth_message.data, dtype=np.uint8).reshape(depth_message.height, depth_message.width, -1)
        # depth = depth_message.data
        print('data get')
        ...
        crop_size = 400
        depth_crop = np.resize(depth[(480-crop_size)//2:(480-crop_size)//2+crop_size, (640-crop_size)//2:(640-crop_size)//2+crop_size], (300, 300))
        print(np.shape(depth_crop))
        # Replace nan with 0 for inpainting.
        depth_crop = depth_crop.copy()
        depth_nan = np.isnan(depth_crop).copy()
        depth_crop[depth_nan] = 0
        cropimg = Image()
        # cropimg.seq = depth_message.seq
        # cropimg.stamp = depth_message.stamp
        # cropimg.frame_id = depth_message.frame_id
        cropimg.height = depth_message.height
        cropimg.width = depth_message.width
        cropimg.encoding = depth_message.encoding
        cropimg.is_bigendian = depth_message.is_bigendian
        cropimg.step = depth_message.step
        img = img1.astype(np.uint8)
        img = img.tolist()
        cropimg.header = depth_message.header
        print(type(img))
        cropimg.data = img
        depth_message.data = img
        print("print:",np.shape(img))
        grasp_crop.publish(depth_message)
    with TimeIt('Inpaint'):
