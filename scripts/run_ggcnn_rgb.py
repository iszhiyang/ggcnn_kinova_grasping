...
import ggcnn4_rgb  # code for the GG-CNN network
...
# Load Network
MODEL_FILE = '/home/name/catkin_ws/src/ggcnn_kinova_grasping/scripts/ggcnn_weights_cornell/cornell_rgb_480/epoch_37_iou_0.93_statedict.pt'
# MODEL_FILE ='/home/name/catkin_ws/src/ggcnn_kinova_grasping/scripts/ggcnn_weights_cornell/jacquard_rgb_480/epoch_50_iou_0.86_statedict.pt'
...
# Define image size
crop_size = 720 # 400 default
re_size = 720 # 300 default
# Tensorflow graph to allow use in callback.
# graph = tf.get_default_graph()   // log
# graph = tf.compat.v1.get_default_graph()

# Get the camera parameters
camera_info_msg = rospy.wait_for_message('/realsense/color/camera_info', CameraInfo)
K = camera_info_msg.K
fx = K[0]
cx = K[2]
fy = K[4]
...
def depth_callback(depth_message):
    ...
    with TimeIt('Crop'):
        depth = bridge.imgmsg_to_cv2(depth_message,"bgr8")
        depth_copy = np.array(depth,dtype=np.uint8)
        # print(depth.max())
        depth = depth/256
        mean = depth.mean()
        depth = (depth - mean)/12
        depth_crop = depth
        depth_img = (depth_crop * 12 +mean)*256
        depth_img = np.array(depth_img,dtype=np.uint8)
        depth_pub.publish(bridge.cv2_to_imgmsg(depth_img))

        depth_crop = cv2.resize(depth[(720-crop_size)//2:(720-crop_size)//2+crop_size, (1280-crop_size)//2:(1280-crop_size)//2+crop_size], (re_size, re_size))
    ...
    with TimeIt('Inference'):
        ...
        # depth_crop = depth_crop/depth_crop.max() # resize to (.)
        # depth_crop = np.clip((depth_crop - depth_crop.mean() ), -1, 1) # threshold
        # print('Mean is:', depth_crop.mean())
        # depth_crop = np.clip((depth_crop - 410), -1, 1)
        depth_input = depth_crop.copy()
        print(depth_input.min(),depth_input.max())
    ...
    with TimeIt('Control'):
        ...
        max_pixel = ((np.array(max_pixel) / float(re_size) * crop_size) + np.array([(480 - crop_size)//2, (640 - crop_size) // 2]))
    ...
    with TimeIt('Draw'):
        ...
        grasp_img = np.zeros((re_size, re_size, 3), dtype=np.uint8)
        # grasp_img = np.zeros((720, 1280, 3), dtype=np.uint8)
        grasp_img[:,:,2] = (points_out * 255.0) # confidence?
        print('point out max:',points_out.max())
    ...
    with TimeIt('Publish'):
        ...
        # print('Ang:',ang,' Width:',width)
        cmd_msg.data = [ang, width]
...
depth_sub = rospy.Subscriber('/realsense/color/image_raw', Image, depth_callback, queue_size=1)
