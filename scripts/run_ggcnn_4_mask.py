...
import ggcnn4  # code for the GG-CNN network
...
# Load Network
# MODEL_FILE = '/home/name/catkin_ws/src/ggcnn_kinova_grasping/scripts/ggcnn_weights_cornell/cornell_d_480/epoch_39_iou_0.77_statedict.pt'
MODEL_FILE ='/home/name/catkin_ws/src/ggcnn_kinova_grasping/scripts/ggcnn_weights_cornell/jacquard_d_480/epoch_45_iou_0.91_statedict.pt'
...
# Define image size
crop_size = 480 # 400 default
re_size = 480 # 300 default
...
def depth_callback(depth_message):
    ...
    with TimeIt('Crop'):
        ...
        # if depth_max > 1000:
        print('depth max is:', depth_max)
        # depth = depth/500
        # depth -=1
        # print(np.shape(depth))
        # Crop a square out of the middle of the depth and resize it to 300*300
        # crop_size = 400
        depth_crop = cv2.resize(depth[(480-crop_size)//2:(480-crop_size)//2+crop_size, (640-crop_size)//2:(640-crop_size)//2+crop_size], (re_size, re_size))
        # print(depth.min(),depth.max())
        # Replace nan with 0 for inpainting.
        depth_crop = depth_crop.copy()
        depth_crop_copy = depth_crop
    ...
    with TimeIt('Filter'):
        # Filter the outputs.
        depth_copy_mask = np.zeros_like(depth_crop_copy)
        print('max:',depth_crop_copy.max())
        depth_copy_mask[(depth_crop_copy/1000) > 0.2]=1
        print('sum is:',sum(depth_copy_mask))
        points_out = points_out * depth_copy_mask
