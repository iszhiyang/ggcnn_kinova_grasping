...
def execute_grasp():
    ...
    print("grasp in right_base_link frame:",gp_base)
    publish_pose_as_transform(gp_base, 'right_base_link', 'G', 0.5)
    # get state of left end effector
    print("waiting for state of right_ee_link")
    right_ee_state = rospy.wait_for_message('/right_ee_in_right_base', geometry_msgs.msg.Transform)
    trans = right_ee_state.translation
    print("Position of right_ee_link",trans)
    ee = [trans.x, trans.y, trans.z]
    diff_x = gp_base.position.x - ee[0]
    diff_y = gp_base.position.y - ee[1]
    diff_z = gp_base.position.z - ee[2]
    diff = [diff_x, diff_y, diff_z]
    print('Difference from right end effector to grasp point',diff)
