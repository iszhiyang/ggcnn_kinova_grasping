...
'''
safe_work_space = [0.53, 0.86, -0.48, -0.02, 0.95, 1.15]
desktop_depth = 0.53
...
class movo_ggcnn():
    ...
    def __init__(self):
        self.max_velo = [0.25, 0.15, 0.085] # for cartesian movement
        self.max_velo_rot = 2 # for rotation
        ...
        self.gp_goal = [0, 0, 0, 0] # infered grasping point in right_base_link [x, y, z, y_up]
        ...
        print('hello')
        self.gripper_velo_pub = rospy.Publisher('/test_vel', std_msgs.msg.Float32, queue_size=1)
        self.velo_pub = rospy.Publisher('/test_arm_vel', JacoCartesianVelocityCmd, queue_size=1)
        ...
        # rospy.spin()
        self.vel_publish()
        # self.thread1 = Thread(target= command_sub)
        # self.thread1.start()
        # self.thread2 = Thread(target= position_sub)
        # self.thread2.start()
        # self.thread3 = Thread(target=gripper_position_sub)
        # self.thread3.start()
    def command_callback(self,msg):
        ...
        vy = max(min(dy * 0.4, self.max_velo[1]), -1.0* self.max_velo[1])
    ...
    def grasp_exec(self):
        if self.grasping_done == False:
            ...
            dy = self.gp_goal[1] - self.current_pos_right_base[1] -0.055 # gripper to finger tip offset
            dz = self.gp_goal[2] - self.current_pos_right_base[2]
    
            vx = max(min(dx*2 , self.max_velo[0]), -1.0*self.max_velo[0])
            vy = max(min(dy*2, self.max_velo[1]), -1.0*self.max_velo[1])
